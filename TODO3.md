...
- [ ] 清理 skipped 测试中的旧 wire snapshot。`context.test.ts` 当前 live/current 协议断言已清到 v1.5 `context.splice`，仅保留带旧 `protocol_version` 的 migration fixture；`permission.test.ts` 与 `plan.test.ts` 已审计，文件内没有旧 wire snapshot 或 skip；`compaction/full.test.ts`、`compaction/micro.test.ts` 已清到当前协议。剩余重点是 `resume.test.ts`、`turn.test.ts` 里的旧 record 形状；解除 skip 前先判断是“迁移 fixture”还是“当前协议快照”。
...
- [ ] 收口 Bash 相关剩余 parity：Bash runtime 注册、background gating、plan-mode Bash 普通 permission、approval/cancel/steer 交互，以及 Pre/PostToolUse 成功/失败 cadence、Interrupt hook parity 已由服务层测试覆盖；不再把 Bash builtin 本身列为未迁移。剩余重点是 PermissionRequest/PermissionResult observer hooks、PreToolUse 等待期间 cancel 不执行 Bash，以及 UserPromptSubmit skipped hook parity。覆盖测试：`permission.test.ts`、`plan.test.ts`、`tool.test.ts`、`turn.test.ts`。
- [ ] 将仍缺服务层 model-tool 入口的旧 Agent-bound tools 迁完：media upload/read、goal tools。`Agent` 已在有 `subagentHost` 的 service runtime 中注册，并覆盖 foreground、resume 参数校验、background gating 和 background spawn；`AskUserQuestion` 已通过复用旧 `AskUserQuestionTool` 的结构化 host 迁到 service runtime，覆盖 foreground question、dismiss、unsupported 分支，并保留旧工具构造路径。注意：`AgentSwarm` 已由 `SwarmModeService` 注册，model-invoked `Skill` 已由 `ModelSkillTool` 注册，`TodoList` / `Cron*` / `EnterPlanMode` / `ExitPlanMode` 也已经是 service-owned tools，不应再列入“未注册 builtin”。覆盖测试：`agent-tool.test.ts`、`question.test.ts`、`tool.test.ts`、`permission.test.ts`、`skill-tool-manager.test.ts`。
...
- [ ] 做一次 full-compaction 服务行为对齐：`compaction/full.test.ts` 的旧 wire snapshot 已清到当前协议，但整文件仍 `describe.skip`。剩余重点是 manual/auto lifecycle、micro-compaction access、OAuth retry、hooks、provider-overflow retry accounting、todo-store integration、history-change cancellation，以及 resume parity。覆盖测试：`compaction/full.test.ts`。
- [ ] 做一次 micro-compaction 服务行为对齐：文件级 skip 已解除，当前有 9 个低风险测试通过，但仍有 16 个 `it.skip` 必须继续消除。剩余重点是 cutoff/projection/telemetry 语义、cache-miss detection、restore、undo、公开服务入口与测试基线对齐，以及与 full-compaction 的交互。覆盖测试：`compaction/micro.test.ts`。

## 行为待对齐

- [ ] 恢复 skipped 的 resume projection 覆盖：tool-store 状态恢复已补；goal completion replay card 已补；从历史记录重建 turn counter 已补；pending tool results restore/projection 已补；delayed reminders restore/projection 已补；compaction replay cards 已补。剩余重点是中断 tool call 的合成结果。这里应按 v1.5 canonical records 断言；旧 records 只用于 migration 输入。覆盖测试：`resume.test.ts`、`context.test.ts`、`records/index.test.ts`。
...
- [ ] 排查 `activateAgentServices()` 懒代理“假预热”：审计已完成，裸 `accessor.get(...)` 对 delayed service 不能作为完整预热屏障。已补 `IPermissionRulesService.rules` 强制构造以恢复 session approval replay；已补 `ISwarmMode.isActive` 强制构造以恢复 `AgentSwarm` service-owned tool 注册；已补 `IProfileService.data()`、`IPermissionModeService.mode`、`IPermissionService.data()` 以激活 config / permission restore handlers 和初始 permission mode；已补 `IBackgroundService.list` 强制构造以恢复 background restore/reconcile hooks；已补 `IContextUsageService.getStatus()` 强制构造以恢复 usage restore/context usage hooks；已补 `IAgentSkillService.activate` 强制构造以恢复 `skill.activate` replay handler，覆盖 `skill-tool-manager.test.ts`。restore 预热已补 `IToolStoreService.data()`、`IUserToolService.register`、`IGoalService.getGoal()` 和 `IFullCompaction.isCompacting`。后续优先核对/修复构造期有 wire handler、hook、tool、event listener 的高风险服务：`IPlanModeService`、`IMcpRuntimeService`、`IMicroCompactionService`、`ILoopService`。覆盖测试按对应服务单文件收口，不做一次性大改。
...
- [ ] 恢复 turn-flow hook / event parity：`PreToolUse` / `PostToolUse` 成功路径已补，same-step dedup 的 `PostToolUse` 真实输出也已恢复，`PostToolUseFailure`、`StopFailure` 与 `Interrupt` 已补。剩余重点是 provider finish-reason details、model-not-configured diagnostics、LLM request logging，以及 duplicate tool-call telemetry。覆盖测试：`tool.test.ts`、`turn.test.ts`。
