...
describe('KimiTUI message flow', () => {
  ...
  it('opens /plugins as an interactive manager and toggles plugin MCP servers', async () => {
    const session = makeSession({
      listPlugins: vi.fn(async () => [
        {
          id: 'kimi-datasource',
    ...
    const detail = driver.state.editorContainer.children[0] as PluginDetailSelectorComponent;
  ...
  it('prompts for an install source when /plugins install has no argument', async () => {
    const session = makeSession();
    const { driver } = await makeDriver(session);
    ...
    const dialog = driver.state.editorContainer.children[0] as PluginInstallInputDialogComponent;
  ...
  it('loads a local plugin marketplace file and installs from it', async () => {
    ...
    expect(
      stripSgr(driver.state.editorContainer.children[0]!.render(120).join('\n')),
    ).toContain('Marketplace install or update succeeded. Run /new to apply plugin changes.');
  });
  ...
  it('requires confirmation when removing from a plugin detail picker', async () => {
    const session = makeSession({
      listPlugins: vi.fn(async () => [
        {
          id: 'demo',
